...
int readtmi_(char *jobname, char *f1cgmi,  int *n1b11, float *tmilow,
...
//aa   status = TKopen(granuleID,"1CGMI", TKREAD, "HDF5", jobname, &tkfileinfo, 1);
    status_alpha = TKopen(granuleID,"1CTMI", TKREAD, "HDF5", jobname, &tkfileinfo, 1);
    if (status_alpha != 0)
      {
...
//  SFM  end  04/16/2014
      ...
      for(j=0; j<221; j++)
	{
	  ...
	  if(j>=6 && j<214)
	    {
	      for(k=7;k<9;k++)
		tc[j][k]=data.S3.Tc[j-6][k-7];
	      if(j%2==0)
		{
		  for(k=0;k<2;k++)
		    tc[j][k]=data.S1.Tc[j/2-3][k];
		  for(k=2;k<7;k++)
		    tc[j][k]=data.S2.Tc[j/2-3][k-2];
		}
	      else
		{
		  if(j/2-3<102)
		    {
		      for(k=0;k<2;k++)
			tc[j][k]=0.5*data.S1.Tc[j/2-3][k]+
			  0.5*data.S1.Tc[j/2-2][k];
		      for(k=2;k<7;k++)
			tc[j][k]=0.5*data.S2.Tc[j/2-3][k-2]+
			  0.5*data.S2.Tc[j/2-2][k-2];
		    }
		}
	    }
	}
      ...
      for(j=0; j<221; j++)
	{
	  for(k=0;k<4;k++)
	    {
	      tmihigh[indh]=-999;//data.S2.Tc[j][k]/1.;
	      indh++;
	    }
	  for(k=0;k<9;k++)
	    {
	      tmilow[ind]=tc[j][k];//data.S1.Tc[j][k]/1.;
...
//  SFM  end    12/24/2013
      ...
      for(j=0;j<221;j++)
	{
	  if(j>=6 && j<214)
	    {
	      eiaS1[ipia]=*data.S3.incidenceAngle[j-6];
	      //  eiaS2[ipia]=*data.S2.incidenceAngle[j];
	      xlon[ipia]=data.S3.Longitude[j-6];
	      xlat[ipia]=data.S3.Latitude[j-6];
	      xlonS2[ipia]=data.S3.Longitude[j-6];
	      xlatS2[ipia]=data.S3.Latitude[j-6];
	    }
	  else
	    {
	      xlon[ipia]=-999;
...
void read2akut_(char *jobname, char *f2aku, int *n1c21, 
...
//aa  begin  SFM  09/04/2013
    ...
    //LW 05/04/18 - 2APR
    status = TKopen(&fname[0], "2APR", TKREAD, "HDF5", jobname,
		    &granuleHandle2AKu, 1);
...
//begin  WSO 8/20/14 test SRT to see if it's available and useable
     ...
     else if(L2AKuData.SRT.reliabFactorHY[j] <= 2.9) //Ku sigma-zero in noise; WSO 8/2/18 hybrid flag
       {
...
//begin  WSO 8/2/18 update for hybrid PIA
...
//end    WSO 9/5/13
	  ...
	  srtrelPIAku[ipia]=L2AKuData.SRT.reliabFactorHY[j];  //WSO  8/2/18 hybrid factor
	  //NSRelibFlag[ipia]=L2AKuData.SRT.reliabFlag[j];
	  NSRelibFlag[ipia]=L2AKuData.SRT.reliabFlagHY[j];   //WSO   8/2/18 hybrid flag
...
int readdprtpflag_(char *jobname, char *f2aku, int *n1c21, 
...
//		  &granuleHandle2AKu, 1);
  ...
  //LW 05/04/18 - 2APR
  status_alpha = TKopen(&fname[0], "2APR", TKREAD, "HDF5", jobname,
		  &granuleHandle2AKu, 1);
    if (status_alpha != 0)
      {
       printf("WARNING: Unable to access 2APR %i \n",status_alpha);
       return status_alpha ;
      }
