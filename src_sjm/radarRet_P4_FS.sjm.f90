...
!end    WSO 9/17/13
      ...
      if(ialg==2) then
         ...
         call copycolumncloudliqsigmas1_fs(dprdata%cloudLiqSigma(i,j),i-1)
         cldiprof = missing_r4
         ...
         call copysfcrainliqfracs1_fs(sfcRainLiqFrac(i, j), i-1)
...
!end    WSO 8/19/13
      ...
      if(ialg==2) then 
         ...
         call copyenvtemps1_fs(dPRData%modTemp(:,i,j),env_nodes(:,i),i-1)
         call copysfcairtemps1_fs(dPRData%modSfcTemp(i,j),i-1)
         call copyenvpresss1_fs(dPRData%envPress(:,i,j),env_nodes(:,i),i-1)
         call copysfcairpresss1_fs(dPRData%envSfcPress(i,j),i-1)
         call copyenvqvs1_fs(dPRData%modQv(:,i,j),env_nodes(:,i),i-1)
         call copyenvsfqvs1_fs(dPRData%modQv(:,i,j),i-1)
         call copyskintemps1_fs(dPRData%modSknTemp(i,j),i-1)
         call copyskintempsigmas1_fs(dprdata%skinTempSigma(i, j), i-1)
         call copycolumnvaporsigmas1_fs(dprdata%tpwSigma(i, j), i-1)
         call copycolumncloudliqsigmas1_fs(dprdata%cloudLiqSigma(i,j), i-1)
         call copyalgotypes1_fs(algotype_NS(i, j), i-1)
         call copyerrorofdatafits1_fs(dprdata%chiSq(i, j), i-1)
      endif
!end    WSO 8/30/13
      !stype = LUT%land_class_map(mod(floor((DPRData%xlon(i,j)+180.)/360.*5760.),5760)+1, 2880-floor((DPRData%xlat(i,j)+90.)/180.*2880.)) !SJM 9/9/15'
      jj=2880-floor((DPRData%xlat(i,j)+90.)/180.*2880.)
...
!begin  WSO 9/28/13 add rain flag including missing for bad scans
        call copyrainflags2_fs(dPRData%rainFlagBad(i,j), i-1)
!end    WSO 9/28/13
...
!begin  WSO 9/15/13 add missing for cloud ice profiles
        ...
        call copycolumncloudliqsigmas2_fs(dprdata%cloudLiqSigma(i,j),i-1)
        cldiprof = missing_r4
...
!begin  WSO 8/7/13
         ...
         call copysfcrainliqfracs2_fs(sfcRainLiqFracMS(i, j), i-1)
!end    WSO 8/7/13
...
!         call copyenvsftemps2_fs(dPRData%envTemp(i,j),i-1)
         ...
         call copyskintempsigmas2_fs(dprdata%skintempSigma(i, j), i-1)
         call copycolumnvaporsigmas2_fs(dprdata%tpwSigma(i, j), i-1)
         call copycolumncloudliqsigmas2_fs(dprdata%cloudLiqSigma(i,j), i-1)
         call copyalgotypes2_fs(algotype_MS(i, j), i-1)
         call copyerrorofdatafits2_fs(dprdata%chiSq(i, j), i-1)
!end    WSO 8/30/13
...
!end    WSO 10/14/15
         ...
         call copysfcemissouts2sigma(emis_rms_MS(i,j,:),i-1)
      endif
