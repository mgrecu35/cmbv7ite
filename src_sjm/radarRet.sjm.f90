...
!begin  WSO 9/15/13 set to flag instead of 0
  d03D = missing_r4
  zcKu3D = missing_r4
  piaOut = dprdata%piaKuNonRain!missing_r4
!end    WSO 9/15/13 
  sfcRain=0.
  sfcRainStd=0.
  dPRRet%z35mod0=0.
...
!begin  WSO 9/15/13 set to flag instead of 0
  ...
  zcKu3DMS = missing_r4
  zcKa3DMS = missing_r4
  piaOutKuMS = dprdata%piaKuNonRain!missing_r4
  piaOutKaMS = dprdata%piaKaNonRain!missing_r4
!end    WSO 9/15/13 
  sfcRainMS=0.
  sfcRainStdMS=0.
!begin  WSO 9/15/13 set to flag instead of 0
...
!end    WSO 9/17/13
         ...
         !copy from 1DVAR (SJM 10/5/2020)
         !cldwprof=0
         cldwprof = dprdata%modCloud(:,i,j)
      endif
      if(ialg==2) then
         call copycldwaters1t(cldwprof,i-1)
         call copycolumncloudliqsigmas1t(dprdata%cloudLiqSigma(i,j),i-1)
         ...
         call copycolumncloudliqsigmas1(dprdata%cloudLiqSigma(i,j),i-1)
         cldiprof = missing_r4
...
!end    WSO 8/19/13
      ...
      if(ialg==2) then 
         ...
         call copyw10t(w10_out_NS(i,j),i-1) !modified SJM 12/4/2014 - need to copy 1DVAR field
         call copyw10sigmat(w10_rms_NS(i,j),i-1) !modified SJM 12/4/2014 - need to copy 1DVAR field
         !begin  WSO 8/30/13 update to specify environmental nodes
         call copyenvtemps1t(dPRData%modTemp(:,i,j),env_nodes(:,i),i-1)
         call copysfcairtemps1t(dPRData%modSfcTemp(i,j),i-1)
         call copyenvpresss1t(dPRData%envPress(:,i,j),env_nodes(:,i),i-1)
         call copysfcairpresss1t(dPRData%envSfcPress(i,j),i-1)
         call copyenvqvs1t(dPRData%modQv(:,i,j),env_nodes(:,i),i-1)
         call copyenvsfqvs1t(dPRData%modQv(:,i,j),i-1)
         call copyskintemps1t(dPRData%modSknTemp(i,j),i-1)
         !call copyskintempsigmas1t(skintempsigma_NS(i, j), i-1)
         call copyskintempsigmas1t(dprdata%skinTempSigma(i, j), i-1)
         call copycolumnvaporsigmas1t(dprdata%tpwSigma(i, j), i-1)
         call copycolumncloudliqsigmas1t(dprdata%cloudLiqSigma(i,j), i-1)
         call copyalgotypes1t(algotype_NS(i, j), i-1)
         call copyerrorofdatafits1t(dprdata%chiSq(i, j), i-1)
      else
         ...
         call copyenvtemps1(dPRData%modTemp(:,i,j),env_nodes(:,i),i-1)
         call copysfcairtemps1(dPRData%modSfcTemp(i,j),i-1)
         call copyenvpresss1(dPRData%envPress(:,i,j),env_nodes(:,i),i-1)
         call copysfcairpresss1(dPRData%envSfcPress(i,j),i-1)
         call copyenvqvs1(dPRData%modQv(:,i,j),env_nodes(:,i),i-1)
         call copyenvsfqvs1(dPRData%modQv(:,i,j),i-1)
         call copyskintemps1(dPRData%modSknTemp(i,j),i-1)
         !call copyskintempsigmas1(skintempsigma_NS(i, j), i-1)
         call copyskintempsigmas1(dprdata%skinTempSigma(i, j), i-1)
         call copycolumnvaporsigmas1(dprdata%tpwSigma(i, j), i-1)
         call copycolumncloudliqsigmas1(dprdata%cloudLiqSigma(i,j), i-1)
         call copyalgotypes1(algotype_NS(i, j), i-1)
         call copyerrorofdatafits1(dprdata%chisq(i, j), i-1)
      endif
!end    WSO 8/30/13
      !stype = LUT%land_class_map(mod(floor((DPRData%xlon(i,j)+180.)/360.*5760.),5760)+1, 2880-floor((DPRData%xlat(i,j)+90.)/180.*2880.)) !SJM 9/9/15'
      jj=2880-floor((DPRData%xlat(i,j)+90.)/180.*2880.)
...
!begin  WSO 9/15/13 add missing for cloud ice profiles
        ...
        call copycolumncloudliqsigmas2(dprdata%cloudLiqSigma(i,j),i-13)
        cldiprof = missing_r4
...
!         call copyenvsftemps2(dPRData%envTemp(i,j),i-13)
         ...
         call copyskintempsigmas2(dprdata%skinTempSigma(i, j), i-13)
         call copycolumnvaporsigmas2(dprdata%tpwSigma(i, j), i-13)
         call copycolumncloudliqsigmas2(dprdata%cloudLiqSigma(i,j), i-13)
         call copyalgotypes2(algotype_MS(i, j), i-13)
         call copyerrorofdatafits2(dprdata%chisq(i, j), i-13)
!end    WSO 8/30/13
...
!end    WSO 6/5/18 
      ...
      do k=1,13
         !tbout(k)=hFreqPRg(i,j,k-9)
         if(tbout(k) .lt. 0) tbout(k) = dprdata%simTbNonRain(k,i,j) !could also filter this by rain presence, need to compare to obs
      enddo
      !print*, tbout
      if(dPRData%rainType(i,j)>=100) then
         !print*, tbout
...
!!begin MG 09172013
      if(i>12 .and. i<38) then
         ...
         do k=1,13
           !tbout(k)=hFreqPRg(i,j,k-9)
           if(tbout(k) .lt. 0) tbout(k) = dprdata%simTbNonRain(k,i,j)
         enddo
